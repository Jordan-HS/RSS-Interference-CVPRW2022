...
class OFAMobileNetV3(MobileNetV3):
	...
	def sample_active_subnet(self, method="full"):
		...
		if method == "full":
			...
			ks_setting = []
			if not isinstance(ks_candidates[0], list):
				ks_candidates = [ks_candidates for _ in range(len(self.block_group_info))]
			for block, k_set in zip(self.block_group_info, ks_candidates):
				k = random.choice(k_set)
				for _ in block:
					ks_setting.append(k)
			...
			expand_setting = []
			if not isinstance(expand_candidates[0], list):
				expand_candidates = [expand_candidates for _ in range(len(self.block_group_info))]
			for block, e_set in zip(self.block_group_info, expand_candidates):
				e = random.choice(e_set)
				for _ in block:
					expand_setting.append(e)
			...
			# if not isinstance(ks_candidates[0], list):
			# 	ks_candidates = [ks_candidates for _ in range(len(self.block_group_info))]
			k = random.choice(ks_candidates)
			for block in self.block_group_info:
				for _ in block:
					ks_setting.append(k)
			...
			# if not isinstance(expand_candidates[0], list):
			# 	expand_candidates = [expand_candidates for _ in range(len(self.block_group_info))]
			e = random.choice(expand_candidates)
			for block in self.block_group_info:
				for _ in block:
					expand_setting.append(e)
			...
			for _ in self.block_group_info:
				depth_setting.append(d)
	...
	def set_supernet(self):
		self.set_active_subnet(max(self.ks_list), max(self.expand_ratio_list), max(self.depth_list))

	def set_min_net(self):
		self.set_active_subnet(min(self.ks_list), min(self.expand_ratio_list), min(self.depth_list))
