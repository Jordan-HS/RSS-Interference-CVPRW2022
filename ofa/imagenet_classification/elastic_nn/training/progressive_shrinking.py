...
def validate(run_manager, epoch=0, is_test=False, image_size_list=None,
             ks_list=None, expand_ratio_list=None, depth_list=None, width_mult_list=None, additional_setting=None, export_all=False):
	...
	image_size_list = [max(image_size_list)]
	...
	losses_of_subnets, top1_of_subnets, top5_of_subnets, latency_of_subnets, subnets = [], [], [], [], []
	...
	for setting, name in subnet_settings:
		...
		ed = time.time()
		latency_of_subnets.append(ed-st)
		losses_of_subnets.append(loss)
		top1_of_subnets.append(top1)
		top5_of_subnets.append(top5)
		subnets.append(setting)
		valid_log += '%s (%.3f), ' % (name, top1)

	if export_all:
		return losses_of_subnets, top1_of_subnets, top5_of_subnets, valid_log, latency_of_subnets, subnets
	else:
...
def train_one_epoch(run_manager, args, epoch, warmup_epochs=0, warmup_lr=0):
	dynamic_net = run_manager.network	# Run manager holds the network
	...
	with tqdm(total=nBatch,
		...
		for i, (images, labels) in enumerate(run_manager.run_config.train_loader):
			...
			images, labels = images.cuda(), labels.cuda()	# convert to cuda
			target = labels
			...
			for _ in range(args.dynamic_batch_size):
				...
				ks_val, e_val, d_val = run_manager.net_to_decimal(subnet_settings)	
				subnets_trained.append({"ks":ks_val, "e":e_val,"d":d_val})
	...
	return losses.avg.item(), run_manager.get_metric_vals(metric_dict), subnets_trained

# Main function used in training
