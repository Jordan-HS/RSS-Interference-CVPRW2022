...
class RunManager:
    ...
    def load_model(self, model_fname=None):
        ...
        if 'training_acc' in checkpoint:
            self.train_acc = checkpoint['training_acc']
        if 'train_times' in checkpoint:
            self.train_times = checkpoint['train_times']
        if "subnets_trained" in checkpoint:
            self.subnets_trained = checkpoint['subnets_trained']
    ...
    def validate_all_resolution(self, epoch=0, is_test=False, net=None):
        ...
        if isinstance(self.run_config.data_provider.image_size, list):
            ...
            for img_size in [max(self.run_config.data_provider.image_size)]:
    ...
    def train_one_epoch(self, args, epoch, warmup_epochs=0, warmup_lr=0, sample_rate="epoch"):
        ...
        if sample_rate == "2 epoch":
            t_subnets = []
            t_subnets.append(self.network.sample_active_subnet())
            t_subnets.append(self.network.sample_active_subnet())
            ...
            for i, (images, labels) in enumerate(self.run_config.train_loader):
                ...
                if sample_rate == "epoch":
                    ...
                    # compute gradient and do SGD step
                      # or self.optimizer.zero_grad()
                    loss.backward()
                    self.optimizer.step()

                elif sample_rate == "batch":
                    loss_of_subnets = []
                ...
                # self.train_times.append(stop-start)
                t.set_postfix({
                    'loss': losses.avg,
                    **self.get_metric_vals(metric_dict, return_dict=True),
                    'img_size': images.size(2),
    ...
    def train(self, args, method, warmup_epoch=0, warmup_lr=0):
        subnets_trained = []
        for epoch in range(self.start_epoch, self.run_config.n_epochs + warmup_epoch):
            random.seed(time.time())
            if method == "RSS":
                subnet = self.network.sample_active_subnet()
                train_loss, (train_top1, train_top5) = self.train_one_epoch(args, epoch, warmup_epoch, warmup_lr)
                k_val, e_val, d_val = self.net_to_decimal(subnet)
                subnets_trained.append({"ks":k_val,"e":e_val,"d":d_val})

                train_loss, (train_top1, train_top5) = self.train_one_epoch(args, epoch, warmup_epoch, warmup_lr, sample_rate="batch")    
         
            elif method == "RSS Anchor":
                if epoch % 2 == 0:
                    anchor = {"ks":[3]*20, "e":[3]*20, "d":[2]*5}
                else:
                    anchor = {"ks":[7]*20, "e":[6]*20, "d":[4]*5}
                self.network.set_active_subnet(anchor["ks"], anchor['e'], anchor['d'])
                train_loss, (train_top1, train_top5) = self.train_one_epoch(args, epoch, warmup_epoch, warmup_lr)
            else:
                train_loss, (train_top1, train_top5) = self.train_one_epoch(args, epoch, warmup_epoch, warmup_lr)
            ...
            if (epoch + 1) % self.run_config.validation_frequency == 0:
                if method != "supernet":
                    subnet_loss, (subnet_top1, subnet_top5) = self.validate()
                    self.train_acc[0].append(subnet_top1)
                    self.network.set_supernet()

                img_size, val_loss, val_acc, val_acc5 = self.validate_all_resolution(epoch=epoch, is_test=False)

                self.train_acc[1].append(np.mean(val_acc))
            ...
            self.save_model({
                ...
                'training_acc': self.train_acc,
                'train_times': self.train_times,
                'optimizer': self.optimizer.state_dict(),
                'state_dict': self.network.state_dict(),
                'subnets_trained': subnets_trained
            }, is_best=is_best)
    ...
    def eval_subnet_population(self, export, n=50):
        latency = []
        accuracy = []
        subnets = []
        # Warm up
        ...
        for i in range(n):
            subnet = self.network.sample_active_subnet()
            k_val, e_val, d_val = self.net_to_decimal(subnet)
            subnets.append({"ks":k_val,"e":e_val,"d":d_val})

            st = time.time()
            subnet_loss, (subnet_top1, subnet_top5) = self.validate(is_test=False)
            ed = time.time()
            accuracy.append(subnet_top1)
            latency.append(ed-st)

        if export:
            print("Latencies: {}".format(str(latency)))
            print("Accuracies: {}".format(str(accuracy)))
            print("Subnets: {}".format(str(subnets)))
            return latency, accuracy, subnets

    def run_single_net(self, subnet):
        self.network.set_active_subnet(subnet["ks"], subnet["e"], subnet["d"])
        self.reset_running_statistics(self.network)
        _, (subnet_top1, _) = self.validate(is_test=True)

        return subnet_top1

    def get_net_flops(self, subnet):
        flops_predict = Mbv3FLOPsModel(self.network)
        
        subnet = {"ks":[7]*20, "e":[4]*20, "d":[3]*5}
        subnet["image_size"] = self.run_config.data_provider.image_size
        flop = flops_predict.get_efficiency(subnet)
        
        return flop

    def eval_subnet_population(self, buckets, n):
        flops_predict = Mbv3FLOPsModel(self.network)
        full_subnets = []
        acc, flops = [], []

        for bucket in buckets:
            print(f"Running flops bucket: {bucket}")
            subnets_evaluated = 0
            while subnets_evaluated < n:
                subnet = self.network.sample_active_subnet()
                subnet["image_size"] = self.run_config.data_provider.image_size
                
                if bucket == min(buckets):
                    subnet["e"] = [3]*20
                    
                elif bucket == max(buckets):
                    subnet["e"] = [6]*20
                    
                flop = flops_predict.get_efficiency(subnet)

                while flop < bucket-0.5 or flop > bucket+0.5:
                    subnet = self.network.sample_active_subnet()
                ...
                full_subnets.append({"ks":subnet["ks"], "e":subnet["e"], "d":subnet["d"]})
                self.network.set_active_subnet(subnet["ks"], subnet["e"], subnet["d"])
                subnet = self.network.get_active_subnet(preserve_weight=True)
                
                self.reset_running_statistics(self.net)
                
                _, (subnet_top1, _) = self.validate(net=self.net, epoch=len(acc), is_test=True)

                acc.append(subnet_top1)
                flops.append(flop)
            
        print(f"subnets: {full_subnets}")
