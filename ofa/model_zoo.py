...
def ofa_net(net_id, pretrained=True):
	if net_id == 'ofa_proxyless_d234_e346_k357_w1.3':
		...
		net = OFAMobileNetV3(
			dropout_rate=0, width_mult=1.0, ks_list=[3, 5, 7], expand_ratio_list=[3, 4, 6], depth_list=[2, 3, 4], n_classes=1000
		)
	...
	elif net_id == 'ofa_mbv3_cifar100':
		net = OFAMobileNetV3(
			dropout_rate=0, width_mult=1.0, ks_list=[3, 5, 7], expand_ratio_list=[3, 4, 6], depth_list=[2, 3, 4], n_classes=100
		)
	else:
	...
	if pretrained:
		...
		# shrink to 100 classes
		# init["classifier.linear.weight"] = init["classifier.linear.weight"][0:100]
		# init["classifier.linear.bias"] = init["classifier.linear.bias"][0:100]
		net.load_state_dict(init)
	return net
